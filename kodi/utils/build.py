...
class Builder:
    def __init__(self):
        ...
        self.execute_current_task_only = False
    ...
    def push_to_repo(self):
        ...
        for arch in self.data['ARCH']:
            if arch['enable']:
                tag = f'{if_build}{self.data["IMAGE_VERSION"]}-{arch["tag"]}'
                push_cmd = f'docker tag {repo}:{tag} lasery/{repo}:{tag}'
                os.system(push_cmd)
                push_cmd = f'docker push lasery/{repo}:{tag}'
                os.system(push_cmd)
...
if __name__ == '__main__':
    ...
    if not (len(sys. argv) > 0 and sys.argv[0] == 'skip'):
        builder.config()
    if len(sys. argv) > 0 and sys.argv[0] == 'only':
        builder.execute_current_task_only = True

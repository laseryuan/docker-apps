...
docker tag ${REPO} lasery/${REPO}:amd64-${VERSION} && docker push lasery/${REPO}:amd64-${VERSION}
docker tag ${REPO}:arm32 lasery/${REPO}:arm32-${VERSION} && docker push lasery/${REPO}:arm32-${VERSION}

docker manifest create -a lasery/${REPO} lasery/${REPO}:amd64-${VERSION} lasery/${REPO}:arm32-${VERSION}

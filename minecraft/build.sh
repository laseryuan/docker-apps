...
nvidia=true mo ./Dockerfile.templ > ./nvidia/Dockerfile
...
docker tag ${REPO}:nvidia lasery/${REPO}:nvidia-${VERSION}
docker push lasery/${REPO}:nvidia-${VERSION}

docker tag ${REPO}:nvidia lasery/${REPO}:nvidia
docker push lasery/${REPO}:nvidia
# docker tag ${REPO}:arm32 lasery/${REPO}:arm32-${VERSION}
# docker push lasery/${REPO}:arm32-${VERSION}

docker manifest create -a lasery/${REPO} lasery/${REPO}:amd64-${VERSION} \
